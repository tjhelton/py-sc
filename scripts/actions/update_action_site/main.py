...
class ActionSiteUpdater:
    ...
    def _build_display(self):
        ...
        stats = Table(title="Action Site Updater", expand=True)
    ...
    async def update_single_action(self, action_id, site_id, live):
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        async with self.semaphore:
            await self.rate_limiter.acquire()

            url = f"{BASE_URL}/tasks/v1/actions/{action_id}/site"
            body = {"site_id": {"value": site_id}}
                ...
                try:
                    async with self.session.put(url, json=body) as response:
                        if response.status == 200:
                            ...
                            self._add_log(
                                f"[green]OK[/green]  {action_id} -> {site_id}"
                            )
    ...
    async def update_all_actions(self, records):
        ...
        with Live(self._build_display(), console=console, refresh_per_second=4) as live:
            for batch_num in range(total_batches):
                ...
                tasks = [
                    self.update_single_action(rec["action_id"], rec["site_id"], live)
                    for rec in batch
                ]
...
def load_input_csv():
    ...
    if not os.path.exists(input_file):
        ...
        console.print("Create input.csv with columns: action_id, site_id")
        return []
    ...
    try:
        ...
        with open(input_file, newline="", encoding="utf-8-sig") as f:
            ...
            if (
                ...
                or "site_id" not in reader.fieldnames
            ):
                console.print(
                    "[red]Error: input.csv must have columns: action_id, site_id[/red]"
                )
                return []
            ...
            for row in reader:
                ...
                if action_id and site_id:
                    records.append({"action_id": action_id, "site_id": site_id})
...
async def main():
    console.rule("SafetyCulture Action Site Updater")
    ...
    async with ActionSiteUpdater() as updater:
